...
static int dmp_stop(struct mldl_cfg *mldl_cfg, void *gyro_handle)
{
	...
	result = inv_mpu6050_serial_read(gyro_handle, mldl_cfg->mpu_chip_info->addr,
	...
	result = inv_mpu6050_serial_single_write(gyro_handle,
				    ...
				    void *mlsl_handle, unsigned char enable)
{
	...
	if (!enable) {
		...
		if (!(reg & BIT_I2C_MST_EN)) {
			result =
			    inv_mpu6050_serial_single_write(
				    mlsl_handle, mldl_cfg->mpu_chip_info->addr,
				    MPUREG_USER_CTRL,
				    (reg | BIT_I2C_MST_EN));
			if (result) {
		...
		struct mldl_cfg *mldl_cfg, void *mlsl_handle)
{
	...
	prod_ver &= 0xF;
	result = inv_mpu6050_serial_read_mem(mlsl_handle, mldl_cfg->mpu_chip_info->addr,
				     memAddr, 1, &prod_rev);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
				 ...
				 int slave_id)
{
	...
	result = inv_mpu6050_serial_read(gyro_handle,
				mldl_cfg->mpu_chip_info->addr,
...
int inv_mpu6050_set_firmware(struct mldl_cfg *mldl_cfg, void *mlsl_handle,
			 const unsigned char *data, int size)
{
	...
	for (bank = 0; size > 0; bank++,
		...
		result = inv_mpu6050_serial_write_mem(mlsl_handle,
				mldl_cfg->mpu_chip_info->addr,
				((bank << 8) | 0x00),
				write_size,
				data);
		...
		result = inv_mpu6050_serial_read_mem(mlsl_handle,
				mldl_cfg->mpu_chip_info->addr,
				((bank << 8) | 0x00),
				write_size,
				read);
		       ...
		       unsigned long sensors)
{
	...
	/* Write and verify memory */
#if INV_CACHE_DMP != 0
	inv_mpu6050_set_firmware(mldl_cfg, gyro_handle,
	...
	result = inv_mpu6050_serial_write(gyro_handle, mldl_cfg->mpu_chip_info->addr,
				  7, regs);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
...
int mpu6050_gyro_config(void *mlsl_handle,
...
int mpu6050_gyro_get_config(void *mlsl_handle,
		struct mldl_cfg *mldl_cfg,
		struct ext_slave_config *data)
{
	struct mpu_gyro_cfg *mpu_gyro_cfg = mldl_cfg->mpu_gyro_cfg;
 ...
 *         by this software.
 */
int inv_mpu6050_open(struct mldl_cfg *mldl_cfg,
		 void *gyro_handle,
		 void *accel_handle,
 ...
 * @return INV_SUCCESS or non-zero error code
 */
int inv_mpu6050_close(struct mldl_cfg *mldl_cfg,
		  void *gyro_handle,
		  void *accel_handle,
		  void *compass_handle,
 ...
 */
int inv_mpu6050_resume(struct mldl_cfg *mldl_cfg,
		   void *gyro_handle,
		   void *accel_handle,
		   void *compass_handle,
		   void *pressure_handle,
		   unsigned long sensors)
{
	...
	mpu6050_mldl_print_cfg(mldl_cfg);
 ...
 *  @return  INV_SUCCESS or a non-zero error code.
 */
int inv_mpu6050_suspend(struct mldl_cfg *mldl_cfg,
		    void *gyro_handle,
		    void *accel_handle,
		    void *compass_handle,
		    void *pressure_handle,
...
int inv_mpu6050_slave_read(struct mldl_cfg *mldl_cfg,
		       void *gyro_handle,
		       void *slave_handle,
		       struct ext_slave_descr *slave,
...
int inv_mpu6050_slave_config(struct mldl_cfg *mldl_cfg,
			 void *gyro_handle,
			 void *slave_handle,
			 struct ext_slave_config *data,
			 struct ext_slave_descr *slave,
...
int inv_mpu6050_get_slave_config(struct mldl_cfg *mldl_cfg,
			     void *gyro_handle,
			     void *slave_handle,
			     struct ext_slave_config *data,
			     struct ext_slave_descr *slave,
